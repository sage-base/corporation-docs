...
!!! tip "会議体メンバーの自動生成"
    国会の会議体メンバー（ConferenceMember）は、選挙データ（ElectionMember）から `populate_conference_members.py` スクリプトで自動生成できます。詳細は[会議体メンバー](relations/conference-member.md)を参照してください。
